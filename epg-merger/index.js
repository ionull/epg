...
(async () => {
  try {
    ...
    for (const entry of configEntries) {
      ...
      if (replace_id) console.log('replace_id', url);
      if (!url) continue;
      ...
      try {
        ...
        for (const ch of channelsToProcess) {
          // 获取第一个 display-name 节点
          let nameObj = ch['display-name']?.[0];
          let currentName = "";

          // 判断 nameObj 是字符串还是带属性的对象 (xml2js 的特性)
          if (typeof nameObj === 'string') {
            currentName = nameObj.trim();
          } else if (nameObj && typeof nameObj === 'object' && nameObj._) {
            currentName = nameObj._.trim();
          }
          ...
          if (currentName && displayNameRenames[currentName]) {
            ...
            // 回写修改后的名称到原始数据结构中
            if (typeof nameObj === 'string') {
              ch['display-name'][0] = newName;
            } else {
              ch['display-name'][0]._ = newName;
            }
            
            currentName = newName; // 更新当前变量，以便后续替换 ID 使用
          }
          ...
          if (replace_id && currentName) {
            ...
            ch.$.id = newId; // 更新 channel 对象的 ID 属性
          }
