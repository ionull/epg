...
(async () => {
  ...
  const defaultFrom = now.subtract(1, "day").format("YYYYMMDD");
  ...
  const networks = [
    ...
    { code: "NWDR", name: "NewTV古装" },
    { code: "NWED", name: "NewTV科教" },
    { code: "NWMA", name: "NewTV功夫" },
    { code: "NWHE", name: "NewTV养生" },
    { code: "INDO", name: "The Indonesia Channel" },
  ...
  for (const net of networks) {
    // 根据 code 决定 from 的日期
    const from = ["PCC", "PIN", "PHK"].includes(net.code)
      ? now.subtract(2, "day").format("YYYYMMDD")
      : defaultFrom;
